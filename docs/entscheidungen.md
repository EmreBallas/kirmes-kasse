...
| 52 | Veroeffentlichung | Der Quellcode geht oeffentlich auf GitHub unter der MIT-Lizenz, damit andere Vereine die Kasse nutzen und anpassen koennen. Mitveroeffentlicht: Anwendung, Roadmap, Entscheidungsprotokoll, Drucker-Setup, Technik- und Beschaffungsnotizen, Bildschirmfotos. Nicht veroeffentlicht: die Rohanalyse vom 12.9. (enthaelt interne Fragen zum Verein, liegt lokal unter intern/) und seed.local.json (PIN). Die Test-PIN im Quellcode wurde neutralisiert und die echte PIN aus der Git-Historie entfernt. Automatischer Build: GitHub Actions pruefen jeden Push, ein Tag v* erzeugt ein Release mit fertigem Windows-Paket. | 12.9.2026 | Auftraggeber |
